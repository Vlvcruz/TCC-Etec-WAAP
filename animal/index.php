...
<body>
	...
	<header class="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-3 mb-4 border-bottom">
		...
		$vacinasql = "SELECT v.vacina as vacina FROM vacina_animal va INNER JOIN vacina v ON v.id_vacina = va.id_vacina WHERE id_animal = $_GET[id]";
		$exe = mysqli_query($conexao, $sql);
		$vacinas = "";
		if (mysqli_num_rows($exe) != 1) echo "<script>location.href='index.php'</script>";
				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							switch ($fetch['sexo']) {
								case "m":
									echo " <ion-icon name='male'></ion-icon>, " . $fetch["idade"] . ' Anos';
									break;
								case "f":
									echo "<ion-icon name='female'></ion-icon>, " . $fetch["idade"] . ' Anos';
									break;
							}

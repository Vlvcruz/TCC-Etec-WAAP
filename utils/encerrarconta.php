<?php
    require_once('src/PHPMailer.php');
    require_once('src/SMTP.php');
    require_once('src/Exception.php');
    require_once('conexao.php');

    use PHPMailer\PHPMailer\PHPMailer;
    use PHPMailer\PHPMailer\SMTP;
    use PHPMailer\PHPMailer\Exception;

    $mail = new PHPMailer(true);

    try {
        $mail->SMTPDebug = SMTP::DEBUG_SERVER;
        $mail->isSMTP();
        $mail->Host = 'smtp.gmail.com';
        $mail->SMTPAuth = true;
        $mail->Username = $email;
        $mail->Password = $pass;
        $mail->Port = 587;

        $mail->setFrom($email);
        $mail->addAddress("$_POST[email]");

        $mail->isHTML(true);
        $mail->Subject = 'Conta apagada!';
        $mail->Body="
        <meta charset='utf-8'>
        <div><div class='adM'>
        </div><table width='100%'>
            <tbody><tr>
                <td align='center'>
                    <table border='0' cellpadding='2' cellspacing='0' style='border:2px solid #ececec'>
                        <tbody><tr>
                            <td>
                                <table width='460' cellpadding='0' cellspacing='0' border='0'>
                                    <tbody><tr>
                                        <td bgcolor='#fff' align='middle' style='padding:10px 30px'>
                                            <div style='margin:7px 0px 7px 0px'>
                                                <a href='https://zello.com/' target='_blank'><img src='https://i.imgur.com/qWGv8I4.png' alt='Logotipo WAAP. Clique em ' ativar='' imagens'='' para='' ver='' logotipo'='' border='0' class='CToWUd'></a>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td align='center' bgcolor='#ececec' style='padding:10px 30px'>
                                            <div style='font-family:Roboto,Tahoma,Arial,Helvetica,sens-serif;font-weight:300;color:#3e3e3e;font-size:1.3em;margin:8px 0px 8px 0px'>Equipe <span class='il'>WAAP</span>!
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td bgcolor='#ececec' style='padding:10px 30px'>
                                            <div style='font-family:Roboto,Tahoma,Arial,Helvetica,sens-serif;font-weight:300;color:#6b6b6b;margin:7px 0px 7px 0px;font-size:1.1em'>
                                                Agradecemos por você ter feito parte dessa grande causa, é uma pena, termos de nos despedir, mas saiba que sempre estaremos abertos para ouvir sua avaliação.<br></div>
                                        </td>
                                    </tr>
                                    
                                    <tr>
                                        
                                    </tr>
                                    <tr>
                                        <td bgcolor='#ececec' style='padding:10px 30px'>
                                            <div style='font-family:Roboto,Tahoma,Arial,Helvetica,sens-serif;font-weight:300;color:#6b6b6b;margin:15px 0px 15px 0px;font-size:1.1em'><br>Nossos serviços sempre irão estar a sua disposição trazendo versatilidade e simplicidade para o seu uso pessoal ou para a sua entidade.</div>
                                        </td>
                                    </tr>
                                    
                                    
                                </tbody></table>
                            </td>
                        </tr>
                    </tbody></table>
                </td>
            </tr>
        </tbody></table><div class='yj6qo'></div><div class='adL'>
        </div></div>
        ";
        $mail->AltBody = 'Equipe WAAP';
        $mail->CharSet = 'UTF-8';

        if($mail->send()) {
            echo 'Email enviado com sucesso';
        } else {
            echo 'Email nao enviado';
        }
    } catch (Exception $e) {
        echo "Erro ao enviar mensagem: {$mail->ErrorInfo}";
    }

    

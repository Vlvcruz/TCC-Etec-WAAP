                        ...
                        <div class="row">
                            ...
                            <div class="col-md-1">
                                <div class="form-group">
                                    <label>UF</label>
                                    <input type="text" class="form-control pula" id="uf" name="estado" maxlength="2" value="<?php echo $ong['uf'] ?>" placeholder="UF" readonly>
                                </div>
                            </div>
                                ...
                                $('#cep').on('blur', function() {
                                    ...
                                    if ($.trim($("#cep").val()) != "") {
                                        ...
                                        $.getJSON("https://viacep.com.br/ws/" + $("#cep").val() + "/json/", function(dados) {
                                            ...
                                            $("#uf").val("...");
                                            if (!("erro" in dados)) {
                                                $("#rua").val(unescape(dados.logradouro));
                                                $("#bairro").val(unescape(dados.bairro));
                                                $("#cidade").val(dados.localidade);
                                                $("#uf").val(dados.uf);
                                                if ($.trim($("#whatsapp").val()) == "") {
                                                    $("#whatsapp").val("(" + dados.ddd + ") 9");
                                                }
                                                if ($.trim($("#tel_fixo").val()) == "") {
                                                    $("#tel_fixo").val("(" + dados.ddd + ") ");
                                                }
                                            } else {
                                                $("#rua").val("");
                                                $("#bairro").val("");
                    ...
                    <form action="" method="post">
                        ...
                        <div class="row">
                            <div class="col">
                                ...
                                <figcaption class="figure-caption">(DDD) 99999-9999</figcaption>
                            </div>
                            <div class="col">
                                ...
                                <figcaption class="figure-caption">(DDD) 9999-9999</figcaption>
                            </div>
                        </div>
                        <br>
...
if (isset($_POST['enviar'])) {
    ...
    if (empty($_POST['cep']) || empty($_POST['num'])) {
        ...
        if (strlen($_POST['cep']) != 9) {
            ...
            if ($uf != "SP") {
                echo "<script>
                    alert('Sentimos muito, o WAAP não está disponivel na sua região atualmente.');
                </script>";
                exit;
            }
            //echo "<script>alert($uf);</script>";exit;
    ...
    if (empty($_POST['email']) || empty($_POST['tel_fixo'])) {
        ...
        } else if (strlen($_POST['whatsapp']) != 15 && !empty($_POST['whatsapp']) && $_POST['whatsapp'] != "(11) 9") {
            echo "
                <script>
    ...
    if ($exe) {
        ...
        $exefoto = mysqli_query($conexao, "SELECT logo,foto_capa FROM ong WHERE id_ong = $_SESSION[id_ong]");
        $fetchfoto = mysqli_fetch_array($exefoto);
        unlink("../../assets/ong/logo/$fetchfoto[logo]");
        if ($fetchfoto['foto_capa'] != "") {
            unlink("../../assets/ong/capa/$fetchfoto[foto_capa]");
        }
